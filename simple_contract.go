...
type AssetState struct {
	Id     *string           `json:"id,omitempty"`     // all assets must have an ID, primary key of contract
	Zone    *float64         `json:"zone,omitempty"`    // current asset location
...
func (t *SimpleChaincode) validateInput(args []string) (stateIn AssetState, err error) {
	...
	if stateIn.Id != nil {
		assetID = strings.TrimSpace(*stateIn.Id)
		if assetID == "" {
